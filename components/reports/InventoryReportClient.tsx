...
import { fetchFinishedGoodsInventoryReport } from "@/actions/reports";
import type { FinishedGoodsInventoryReportRow } from "@/lib/reports/queries";
import { Button } from "@/components/ui/button";
import { FinishedGoodsInventoryXLSXButton } from "@/components/reports/FinishedGoodsInventoryXLSXButton";
...
export function InventoryReportClient({
  ...
  const [reportData, setReportData] = useState<FinishedGoodsInventoryReportRow[]>(
    []
  );
  ...
  const loadReport = useCallback(async () => {
    if (!productId || rangeError) return;
    setIsLoading(true);
    try {
      const data = await fetchFinishedGoodsInventoryReport({
        productId,
        fromMonth,
        fromYear,
        toMonth,
  ...
  }, [productId, fromMonth, fromYear, toMonth, toYear, rangeError]);

  useEffect(() => {
    void loadReport();
    // Initial load only — Update button calls loadReport() explicitly
  ...
  const rowsWithSeparators = useMemo(() => {
    const out: Array<
      ...
      | { kind: "row"; row: FinishedGoodsInventoryReportRow }
    > = [];
  ...
  return (
    <>
        ...
        <div className="grid gap-4 md:grid-cols-4">
          ...
          <div className="flex items-end">
            <Button
              ...
              disabled={isLoading || !productId || Boolean(rangeError)}
            >
      ...
      <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
        ...
        <FinishedGoodsInventoryXLSXButton
          rows={exportableRows}
                ...
                {rowsWithSeparators.map((item) => {
                  ...
                  return (
                    <tr key={r.dateISO}>
                      ...
                      <td style={{ ...cellStyle, textAlign: "right", fontVariantNumeric: "tabular-nums" }}>
                        {isFuture ? "-" : r.production.toFixed(2)}
                      </td>
                      <td style={{ ...cellStyle, textAlign: "right", fontVariantNumeric: "tabular-nums" }}>
