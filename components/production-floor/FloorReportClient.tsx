...
import { getFloorReportRange } from "@/actions/production-floor";
import { Button } from "@/components/ui/button";
import { FloorReportXLSXButton } from "@/components/production-floor/FloorReportXLSXButton";
...
export function FloorReportClient({
  materials,
  defaultMaterialId,
}: {
  ...
  const [reportData, setReportData] = useState<FloorReportRow[]>([]);
  const [isLoading, setIsLoading] = useState(false);

  const fromKey = monthYearKey(fromMonth, fromYear);
  const toKey = monthYearKey(toMonth, toYear);
  const rangeError = toKey < fromKey ? "End month cannot be before start month." : null;
  ...
  const loadReport = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const data = await getFloorReportRange({
      ...
      {isLoading ? (
        ...
        <div className="rounded-md border bg-card px-4 py-8 text-center">
          <p className="text-sm text-muted-foreground">
            No floor ledger activity found for the selected range.
          </p>
        </div>
      ) : (
        <div
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ backgroundColor: "hsl(var(--card))" }}>
                  {["Date", "Opening", "Issued", "Consumed", "Adjusted", "Closing"].map((label) => (
                    <th
                      key={label}
                      style={{
                        position: "sticky",
                ...
                {rowsWithSeparators.map((item) => {
                  ...
                  return (
                    <tr key={r.dateISO}>
                      ...
                      <td style={{ ...cellStyle, textAlign: "right", fontVariantNumeric: "tabular-nums" }}>
                        {isFuture ? "-" : r.consumed.toFixed(2)}
                      </td>
                      <td style={{ ...cellStyle, textAlign: "right", fontVariantNumeric: "tabular-nums" }}>
                        {isFuture ? "-" : r.adjusted.toFixed(2)}
                      </td>
                      <td style={{ ...cellStyle, textAlign: "right", fontVariantNumeric: "tabular-nums" }}>
