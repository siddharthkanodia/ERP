...
import { WorkOrderReadyBanner, WorkOrderShell } from "./work-order-shell";

const EDIT_FORM_ID = "work-order-edit-form";
...
export default async function WorkOrderDetailPage({
  ...
  const [workOrder, session] = await Promise.all([
    getWorkOrderById(id),
    getAuthSession(),
  ]);
  ...
  const unit = workOrder.finishedProductUnit ?? "KG";
        ...
        <div>
          ...
          <p
            className={`font-medium ${
              workOrder.variance < 0 ? "text-red-600" : "text-emerald-700"
            }`}
          >
        ...
        <div className="rounded-md border bg-card p-3 text-sm">
          <div className="mb-2 flex items-center justify-between">
            <p className="text-muted-foreground">
              Floor Stock (selected materials)
            </p>
            {!isReadOnly ? (
              <Link
          ...
          {zeroStockMaterials.length > 0 && !isReadOnly ? (
            <p className="mt-2 text-xs font-medium text-amber-600">
              Warning:{" "}
              {zeroStockMaterials.map((m) => m.name).join(", ")} has 0 floor
              stock. Issue stock to the production floor before producing.
            </p>
          ) : null}
        </div>
      ) : null}
