...
export async function consumeFromFloorBatch(
  ...
  try {
    await prisma.$transaction(async (tx) => {
      for (const item of input.items) {
        const floor = await tx.productionFloorStock.findUnique({
...
export async function getFloorLedgerEntries(params: {
  ...
  const entries = await prisma.productionFloorLedger.findMany({
    where: params.rawMaterialId ? { rawMaterialId: params.rawMaterialId } : undefined,
...
export async function updateFloorLedgerEntry(
  ...
  try {
    await prisma.$transaction(async (tx) => {
      const existing = await tx.productionFloorLedger.findUnique({
        where: { id: entryId },
      ...
      await rebuildFloorBalancesForMaterial(tx, existing.rawMaterialId);
    });
  } catch (error: unknown) {
    const code =
      typeof error === "object" && error !== null && "code" in error
